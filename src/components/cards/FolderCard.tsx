...
export function FolderCard({ folder }: FolderCardProps) {
  ...
  return (
    <Card className="group hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:shadow-md transition-all duration-200 cursor-pointer" onClick={() => goToFolder(folder?._id)}>
        ...
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-muted-foreground truncate"> {folder?.stats?.totalItems} {folder?.stats?.totalItems === 1 ? 'item' : 'items'}
              </p>
            </div>
